...
while (<DATA>) {
   ...
   $foo = $u->canonical;
   if ($foo ne $uref) {
       push(@rel_fail, qq($testno: URI::XS->new("$expect", "$base")->rel ==> "$foo" (not "$uref")\n));
   }
...
# the apache parser and therefor URI::XS doesn't thing scheme should apear without "://"
#  I added the following two examples back in, since they now pass the
#  recommented parsing method, now that we handle non-hierarchical URI
      http:g        =  http:g
      http:         =  http:
