...
struct apr_uri_t {
    ...
    char *raw;      /* raw uri string */
    struct hostent *hostent; /* structure returned from gethostbyname() */

    apr_port_t port; /* The port number, numeric, valid only if port_str != NULL */
    
    unsigned is_initialized:1; /* has the structure been initialized */
    unsigned dns_looked_up:1;  /* has the DNS been looked up yet */
    unsigned dns_resolved:1;   /* has the dns been resolved yet */
 ...
 */
char * apr_uri_unparse(const apr_uri_t *uptr,
		       unsigned flags);
 ...
 */
int apr_uri_parse(const char *uri, 
		  apr_uri_t *uptr);
 ...
 * @return APR_SUCCESS for success or error code
 */
int apr_uri_parse_hostinfo(const char *hostinfo, 
			   apr_uri_t *uptr);
